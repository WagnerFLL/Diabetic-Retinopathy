...
# Import data
l = pd.read_csv("../labels/trainLabels_master_256_v2.csv")
labels = l.head(52000)
das = np.load("../data/X_train.npy")
X = das[:52000]
y = np.array([1 if l >= 1 else 0 for l in labels['level']])
# y = np.array(labels['level'])

print(labels[labels["level"]==0].shape[0])
...
model, hist = cnn_model(X_train, X_test, y_train, y_test, kernel_size, nb_filters, channels, nb_epoch, batch_size, nb_classes)
